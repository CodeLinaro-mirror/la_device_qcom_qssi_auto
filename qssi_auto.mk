#For QSSI_auto, we build only the system image. Here we explicitly set the images
#we build so there is no confusion.

#Call Twowheeler mk to set flags & copy etc files needed
-include $(TOPDIR)device/qcom/qssi_auto/twowheeler.mk
...
# after QSSI_auto has been merged with the SoC build.
...
VENDOR_QTI_PLATFORM := qssi_auto
VENDOR_QTI_DEVICE := qssi_auto
...
$(call inherit-product, device/qcom/qssi_auto/common64.mk)
...
TARGET_SYSTEM_PROP += device/qcom/qssi_auto/system.prop
...
DEVICE_FRAMEWORK_MANIFEST_FILE := device/qcom/qssi_auto/framework_manifest.xml
...
PRODUCT_COPY_FILES += \
    device/qcom/qssi_auto/init.qcom.testscripts.sh:$(TARGET_COPY_OUT_PRODUCT)/etc/init.qcom.testscripts.sh
endif

PRODUCT_COPY_FILES += \
    device/qcom/qssi_auto/public.libraries.product-qti.txt:$(TARGET_COPY_OUT_PRODUCT)/etc/public.libraries-qti.txt

# copy system_ext specific whitelisted libraries to system_ext/etc
PRODUCT_COPY_FILES += \
    device/qcom/qssi_auto/public.libraries.system_ext-qti.txt:$(TARGET_COPY_OUT_SYSTEM_EXT)/etc/public.libraries-qti.txt
...
DEVICE_PACKAGE_OVERLAYS += device/qcom/qssi_auto/overlay
endif
...
# Include mainline components and qssi_auto whitelist
ifeq (true,$(call math_gt_or_eq,$(SHIPPING_API_LEVEL),29))
  $(call inherit-product, device/qcom/qssi_auto/qssi_auto_whitelist.mk)
  PRODUCT_ARTIFACT_PATH_REQUIREMENT_IGNORE_PATHS := /system/system_ext/
  PRODUCT_ENFORCE_ARTIFACT_PATH_REQUIREMENTS := true
endif

# Enable support for APEX updates
# Disable support for APEX updates for 2w
...
# Includes TwoWheeler Vehicle HAL extension.
PRODUCT_PACKAGES += \
